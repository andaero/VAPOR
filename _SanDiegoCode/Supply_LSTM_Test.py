...
importSupplyDf = pd.read_csv("../Data/supplyData.csv", parse_dates=["DateTime"])
...
importSupplyDf[column_list].clip(lower=0,inplace=True) #replace neg nums w 0
...
# Remove outliers
supplyDf = supplyDf.drop(["DateTime"], axis=1)
df = supplyDf[(np.abs(stats.zscore(supplyDf)) < 3).all(axis=1)]
...
scaler = MinMaxScaler()
df["SupplyTotal"] = scaler.fit_transform(df["SupplyTotal"].values.reshape(-1,1))
...
class SaveBestModel(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if(logs[self.save_best_metric] < self.lowestLoss):
            ...
            self.model.save(f"supply_model/{seq_len}hrinputbidirmodel{round(logs[self.save_best_metric],2)}error.model")
...
for BatchSize in BatchSizes:
    for learning_r in learning_rs:
        for layer in layers:
            model = Sequential()
            NAME = f"V4-Dense Layers-{dense}-SeqLen-{seq_len}Filters-{BatchSize}-Layers{layer}-Learning Rate-{learning_r}-Time-{int(time.time())}"
            ...
            model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=opt, metrics=[tf.keras.metrics.RootMeanSquaredError(),tf.keras.metrics.MeanAbsolutePercentageError()]) #tf.keras.metrics.RootMeanSquaredError()

            model.summary()

            tensorboard = TensorBoard(log_dir=f'SupplyLogsv2/{NAME}', histogram_freq=1, write_images=True)
