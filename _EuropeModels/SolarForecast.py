...
trainDf = pd.read_csv("../EuropeData/train.csv", parse_dates=["time"], usecols=["time", "pv_production", "wind_production", "consumption", "direct_rad_1h:Wh", "diffuse_rad_1h:Wh", "global_rad_1h:Wh", "temp", "wind_speed_2m:ms", "wind_speed_10m:ms"])
print(trainDf)
...
trainDf["target"] = trainDf["pv_production"].shift(-future)
...
scaler = MinMaxScaler()
trainDf["pv_production"] = scaler.fit_transform(trainDf["pv_production"].values.reshape(-1,1))
trainDf["direct_rad_1h:Wh"] = scaler.fit_transform(trainDf["direct_rad_1h:Wh"].values.reshape(-1,1))
trainDf["diffuse_rad_1h:Wh"] = scaler.fit_transform(trainDf["diffuse_rad_1h:Wh"].values.reshape(-1,1))
trainDf["global_rad_1h:Wh"] = scaler.fit_transform(trainDf["global_rad_1h:Wh"].values.reshape(-1,1))
...
for BatchSize in BatchSizes:
    for learning_r in learning_rs:
        for layer in layers:
            model = Sequential()
            NAME = f"V3-Dense Layers-{dense}-Dense Batch-{dBatchSize}-SeqLen-{seq_len}Filters-{BatchSize}-Layers{layer}-Learning Rate-{learning_r}-Time-{int(time.time())}"
            ...
            tensorboard = TensorBoard(log_dir=f'logsv2/{NAME}')
